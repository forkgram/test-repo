...
win:
    ...
    SET FGPATCHES=%ROOT_DIR%\\%REPO_NAME%\\patches
    git -c user.email=build@local -c user.name=build am --ignore-whitespace ^
        "%FGPATCHES%\\0001-Enabled-DirectComposition-in-Win7-path-and-FLIP_SEQU.patch" ^
        "%FGPATCHES%\\0002-Ported-Windows-7-backport-from-Qt-6.10.0-to-Qt-6.11..patch" ^
        "%FGPATCHES%\\0003-Added-three-way-port-script-for-moving-overlay-to-ne.patch" ^
        "%FGPATCHES%\\0004-Routed-new-6.11-notifyRoleChange-UIA-call-through-wr.patch"
""")
